...
const DashboardTutorMessages = () => {
    ...
    const getOtherParticipant = (chat) => {
        return chat.firstPerson === user.username ? chat.secondPerson : chat.firstPerson;
    };
    ...
    const handleMessageSend = async () => {
        ...
        const newMessage = {
            ...
            receiver: getOtherParticipant(chats.find((chat) => chat.chatId === selectedChatId)),
            timestamp: new Date(),
    ...
    useEffect(() => {
        ...
        if (userCookie) {
            ...
            const firstPersonQuery = query(collection(db, "chats"), where("firstPerson", "==", userCookie.username));
            ...
            const secondPersonQuery = query(collection(db, "chats"), where("secondPerson", "==", userCookie.username));
            let combinedChats = [];
    ...
    const getChatMessages = (chatId) => {
        return messages.filter((message) => message.chatId === chatId).sort((a, b) => a.timestamp - b.timestamp);
    };

    const userInvolvedChats = chats.filter((chat) => chat.firstPerson === user.username || chat.secondPerson === user.username);
        ...
        <div className="flex h-[90vh]">
            ...
            <div className="w-1/4 bg-gradient-to-b from-gray-200 to-gray-300 p-4 overflow-y-auto max-h-[90vh]">
                {userInvolvedChats.map((chat) => (
                    <div key={chat.chatId} className={`p-3 mb-2 cursor-pointer rounded-lg ${chat.chatId === selectedChatId ? "bg-gradient-to-r from-purple-500 to-pink-500 text-white" : "hover:bg-gradient-to-r from-purple-300 to-pink-300"}`} onClick={() => handleChatSelect(chat.chatId)}>
                        <p className="font-semibold">{getOtherParticipant(chat)}</p>
                        <p className="text-sm">{getChatMessages(chat.chatId).slice(-1)[0]?.content || "No messages yet"}</p>
                    </div>
                ))}
            </div>
                ...
                <div className="flex-grow overflow-y-auto p-4">
                    {messages &&
                        getChatMessages(selectedChatId).map((message, index) => (
                            <div key={index} className={`flex mb-4 ${message.sender === user.username ? "justify-end" : "justify-start"}`}>
                                <div className={`p-3 rounded-lg ${message.sender === user.username ? "bg-blue-200" : "bg-green-200"} shadow-md`}>
                                    <p>{message.content}</p>
                                </div>
                            </div>
                        ))}
