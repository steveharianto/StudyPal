...
const FindTutor = () => {
  ...
  let filteredTutors = tutors.filter((tutor) => {
    const fullNameIncludesSearch = tutor.fullname
      .toLowerCase()
      .includes(inputSearch.toLowerCase());
    const subjectIncludesSearch = tutor.subject
      .toLowerCase()
      .includes(inputSearch.toLowerCase());
    const subjectIncludesCurrentCategory = tutor.subject
      .toLowerCase()
      .includes(currentCategory.toLowerCase());

    return (
      (fullNameIncludesSearch || subjectIncludesSearch) &&
      subjectIncludesCurrentCategory
    );
  });
  ...
  if (selectedSort === "Trending") {
    filteredTutors = filteredTutors.sort((a, b) => {
      ...
      return selectedSortDirection === "Descending"
        ? classesB.length - classesA.length
        : classesA.length - classesB.length;
    });
  }
  if (selectedSort === "Rating") {
    filteredTutors = filteredTutors.sort((a, b) => {
      ...
      const avgRatingA =
        myRatingsA.length > 0
          ? myRatingsA.reduce((sum, rate) => sum + rate.value, 0) /
            myRatingsA.length
          : 0;
      const avgRatingB =
        myRatingsB.length > 0
          ? myRatingsB.reduce((sum, rate) => sum + rate.value, 0) /
            myRatingsB.length
          : 0;
  ...
  const handleCheckoutButton = async (price: number) => {
    ...
    // Take Balance
    getDocs(
      query(UsersCollectionRef, where("username", "==", currentUser.username))
    )
      .then((querySnapshot) => {
        if (!querySnapshot.empty) {
          ...
          updateDoc(doc(UsersCollectionRef, userDoc.id), {
            balance: newBalance,
          })
            .then(() => {
  ...
  const generateTableRows = () => {
    ...
    const datesOfWeek = currentTutor
      ? getCurrentWeekSchedule(currentTutor.schedule)
      : [];
    ...
    return timeIntervals.map((interval, index) => (
      <tr key={index} className="text-white">
        {daysOfWeek.map((day) => {
          const isInTutorSchedule = currentTutor?.schedule.includes(
            `${day}-${interval.split(" - ")[0].split(":")[0]}`
          );
          const isInSelectedSchedule = selectedSchedule.includes(
            `${day}-${interval.split(" - ")[0].split(":")[0]}`
          );
          // const isAlreadyOrdered = filteredDatesOfWeek.includes(convertTimeStringToTimestamp(`${day}-${interval.split(" - ")[0].split(":")[0]}`));
          const isAlreadyOrdered = filteredDatesOfWeek.some(
            (t) =>
              t.toString() ===
          ...
          if (isInTutorSchedule) {
            currentStyle =
              "rounded hover:bg-blue-300 hover:cursor-pointer transition text-gray-500";
          }
          if (isAlreadyOrdered) {
            currentStyle = "rounded bg-red-200 text-gray-500 ";
          } else if (isInSelectedSchedule && isInTutorSchedule) {
            currentStyle =
              "bg-blue-500 rounded font-medium text-white hover:bg-gray-200 hover:text-gray-500 hover:cursor-pointer transition";
          }
          ...
          return (
            <td
              ...
              onClick={() => {
                if (!isAlreadyOrdered) {
                  currentTutor?.schedule.includes(
                    `${day}-${interval.split(" - ")[0].split(":")[0]}`
                  ) &&
        ...
        <div className="flex justify-between py-2 border-b-2">
          ...
          <div className="flex">
            <p className="font-medium">
              {filteredTutors.length.toLocaleString()}
            </p>{" "}
            <p className="mx-1">results found </p>
          </div>
        </div>
